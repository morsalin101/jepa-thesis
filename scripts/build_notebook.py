...
def build_data_prep(user: str, repo_url: str, branch: str, embed: bool) -> tuple[str, dict, dict]:
    ...
    body = [
        ...
        code(
            ...
            "\n"
            "if not UNLABELED_SRC or not LABELED_SRC:\n"
            "    raise FileNotFoundError(\n"
            "        f'HyperKvasir not found (unlabeled={UNLABELED_SRC}, labeled={LABELED_SRC}).'\n"
            "        '\\n\\nTree:\\n  ' + show_tree() +\n"
            "        '\\n\\nFix: click + Add Input and search for '\n"
            "        '\"hyper-kvasir-unlabeled-images\" and \"hyper-kvasir-labeled-images\" '\n"
            "        '(faisalmahmud69). If they are listed in the sidebar already, '\n"
            "        'hard-reload the page so the session picks them up.')\n"
            "\n"
