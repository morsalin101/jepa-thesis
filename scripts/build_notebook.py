...
def build_data_prep(user: str, repo_url: str, branch: str, embed: bool) -> tuple[str, dict, dict]:
    ...
    body = [
        ...
        code(
            ...
            "import glob\n"
            "\n"
            "print('mounted under /kaggle/input:', sorted(os.listdir('/kaggle/input')) or '(nothing)')\n"
            "\n"
            ...
            "        if hits:\n"
            "            return hits[0]\n"
            "    # Show what IS mounted — the usual cause is a browser session opened\n"
            "    # before the notebook was pushed, so the sidebar still lists the old\n"
            "    # inputs even though the metadata is current. Reload the page.\n"
            "    tree = []\n"
            "    for root, dirs, _ in os.walk('/kaggle/input'):\n"
            "        if root.count(os.sep) - 2 >= 2:\n"
            "            dirs[:] = []\n"
            "            continue\n"
            "        tree.append(root)\n"
            "    raise FileNotFoundError(\n"
            "        'HyperKvasir not mounted. Tried: ' + str(patterns) +\n"
            "        '\\n\\nCurrently mounted:\\n  ' + '\\n  '.join(tree[:40]) +\n"
            "        '\\n\\nFix: reload this page (the input list is cached per browser '\\\n"
            "        'session), or click + Add Input and search for '\n"
            "        '\"hyper-kvasir-unlabeled-images\" and \"hyper-kvasir-labeled-images\".')\n"
            "\n"
