...
def build_segment(user: str, repo_url: str, branch: str, embed: bool) -> tuple[str, dict, dict]:
    ...
    body = [
        ...
        code(
            "for enc in ENCODERS:\n"
            "    for seed in SEEDS:\n"
            "        sh(f'python -m src.engine.segment --encoder {enc} --seed {seed} "
            "--epochs {EPOCHS}', check=False)\n"
        ),
        code(
        ...
        md(
            "## Ablations\n\n"
            "Only runs when `RUN_ABLATIONS = True`. Each block multiplies the run count, "
            "so enable them once the main comparison is settled — not before."
        ),
        code(
            "if RUN_ABLATIONS:\n"
            ...
            "--label-fraction {lf} --seed {seed} --epochs {EPOCHS}', check=False)\n"
            "\n"
            "    # Does the encoder ranking survive a decoder swap?\n"
            "    for enc in ENCODERS:\n"
            "        sh(f'python -m src.engine.segment --encoder {enc} --decoder unet "
            "--seed 0 --epochs {EPOCHS}', check=False)\n"
            "\n"
            "    # 880/120 split, for comparability with published Kvasir-SEG numbers.\n"
            "    for enc in ENCODERS:\n"
            "        sh(f'python -m src.engine.segment --encoder {enc} --split 880_120 "
            "--seed 0 --epochs {EPOCHS}', check=False)\n"
            "else:\n"
